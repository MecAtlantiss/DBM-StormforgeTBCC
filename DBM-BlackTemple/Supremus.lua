...
local warnFixate		= mod:NewTargetNoFilterAnnounce(40892, 3, nil, nil, "Announce Fixate targets")
...
local specWarnFixate	= mod:NewSpecialWarningRun(40892, nil, "Show special warning to run away from Fixate", nil, 4, 2)

local timerPhase		= mod:NewTimer(60, "TimerPhase", 42052, nil, nil, 6)
local nextFixate		= mod:NewTimer(10, "Next fixate", 40892, nil, "Show timer for next Fixate", 6)
...
local function ScanTarget(self)
	...
	if target then
		--if self.vb.lastTarget ~= target then
			...
			if self.vb.fixateTarCount < 5 then
				self.vb.fixateTarCount = self.vb.fixateTarCount + 1
				nextFixate:Start()
			end
