...
mod:RegisterCombat("YELL", "Energy. Power. My people are addicted to it... a dependence made manifest after the Sunwell was destroyed. Welcome to the future. A pity you are too late to stop it. No one can stop me now! Selama ashal'anore!")
mod:SetUsedIcons(1, 6, 7, 8)

mod:RegisterEventsInCombat(
	"SPELL_CAST_START 39427 36819 35941",
...
local warnFear			= mod:NewCastAnnounce(39427, 3)
...
local timerPhoenixCD	= mod:NewCDTimer(55, 36723, "Next Phoenix CD", nil, nil, 1)
local timerRebirth		= mod:NewCDTimer(8.5, 35369, "Egg Respawns", nil, nil, 1)
local timerShieldCD		= mod:NewCDTimer(80, 36815, nil, nil, nil, 4)
...
local timerMindControl  = mod:NewCDTimer(38, 36797, "Next Mind Control", nil, nil, 3)
...
function mod:CHAT_MSG_MONSTER_YELL(msg)
	...
	elseif msg == L.YellPhase4 or msg:find(L.YellPhase4) then
		...
		warnPhase4:Show()
		timerPhase:Cancel()
		timerPhoenixCD:Start(65)
		timerShieldCD:Start(80)
		timerMindControl:Start(18)
	elseif msg == L.YellPhase5 or msg:find(L.YellPhase5) then
		...
		timerGravityCD:Start(55)
		timerPhoenixCD:Start(127)
	end
end
...
function mod:SWING_DAMAGE(_, _, _, _, _, destName)
	if destName == "Phoenix Egg" and self:AntiSpam(15, 1) then
		warnEgg:Show()
		specWarnEgg:Show()
	    specWarnEgg:Play("killmob")
		timerRebirth:Start()
