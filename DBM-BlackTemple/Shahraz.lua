...
local timerFACD			= mod:NewCDTimer(22, 41001, nil, nil, nil, 3)--20-28
...
function mod:OnCombatStart(delay)
	...
	timerFACD:Start(-delay)
	if not self:IsTrivial() then
		self:RegisterShortTermEvents(
			"UNIT_HEALTH boss1"
		)
	end
	if GetDungeonDifficultyID() == 1 then
		berserkTimer:Start(600-delay) --normal
	else
		berserkTimer:Start(480-delay) --heroic
	end
end
...
function mod:UNIT_SPELLCAST_SUCCEEDED(uId, _, spellId)
	if self:AntiSpam(3, spellId) then
		...
		--end
		if spellId == 40869 then--Cast event not in combat log, only applied and that can be resisted (especially on non timewalker). this ensures timer always exists
			timerFACD:Start()
		end
	end
end
